...
public class GravitySimulator {
	...
	public ArrayList<SObject> activeObjects = new ArrayList<SObject>();
	...
	public void enableView() {
		this.view = new View(this.activeObjects, this);
	...
	public void doOperations(int ops) {
		...
		if (this.isCloseRequested) {
			System.out.println("GravitySimulator terminated!");
		} else
	...
	public void logObjects() {
		...
		System.out.println("Simulation state at " + time + "[ts: " + timeStep
				+ "] (" + operations + " ops):");
...
class Logic implements Runnable {
	...
	private void logicTick() {
		...
		for (SObject o1 : gs.activeObjects) {
			...
			for (SObject o2 : gs.activeObjects) {
				if (o1 != o2) {
					...
					ax += (G * o2.getMass() * delta.getX())
							/ Math.pow(r, rExponent);
					ay += (G * o2.getMass() * delta.getY())
							/ Math.pow(r, rExponent);
					az += (G * o2.getMass() * delta.getZ())
							/ Math.pow(r, rExponent);
		...
		for (SObject o : gs.activeObjects) {
			o.setVelocity(new Vec3D(o.getVelocity().getX() + deltaT
					* o.getAcceleration().getX(), o.getVelocity().getY()
					+ deltaT * o.getAcceleration().getY(), o.getVelocity()
					.getZ() + deltaT * o.getAcceleration().getZ()));
		}

		// #3:Position-Schleife:
		for (SObject o : gs.activeObjects) {
			o.setPosition(new Vec3D(o.getPosition().getX()
					...
					+ ((1 / 2) * o.getAcceleration().getY() * Math.pow(deltaT,
							2)), o.getPosition().getZ()
					+ (deltaT * o.getVelocity().getZ())
					+ ((1 / 2) * o.getAcceleration().getZ() * Math.pow(deltaT,
							2))));
		}
